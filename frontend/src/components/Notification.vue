...
const processingId = ref(null);    // pour "marquer comme lu"
const deletingId = ref(null);      // pour "supprimer"

// ---------- Helpers ----------
const getId = (notification) => notification.id || notification._id;
...
const markAsRead = async (notification) => {
  ...
  try {
    await api.patch(`/alertes/${id}/read`);
    notification.is_read = true;
  } catch (err) {
    console.error(err);
    error.value = 'Erreur lors du marquage comme lu.';
  } finally {
...
const markAllAsRead = async () => {
  ...
  try {
    ...
    console.error(err);
    error.value = 'Erreur lors du marquage global.';
  } finally {
...
const deleteNotification = async (notification) => {
  const id = getId(notification);
  // Sauvegarde locale pour rollback en cas d'erreur
  const originalList = [...notifications.value];
  // Optimistic update : suppression locale immédiate
  notifications.value = notifications.value.filter(n => getId(n) !== id);
...
<template>
  <div class="w-full">
    ...
    <div class="flex items-center justify-between mb-6">
      ...
      >
        <span v-if="markingAll">Traitement...</span>
        <span v-else>✓ Tout marquer comme lu</span>
      </button>
    </div>
      ...
      <div class="p-5 border rounded-2xl bg-white/5 border-white/5">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="mt-1 text-2xl font-bold text-white">{{ notifications.length }}</p>
          </div>
          <div class="flex items-center justify-center text-xl rounded-full w-11 h-11 bg-blue-500/10">🔔</div>
        </div>
      </div>
      <div class="p-5 border rounded-2xl bg-white/5 border-white/5">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="mt-1 text-2xl font-bold text-white">{{ unreadCount }}</p>
          </div>
          <div class="flex items-center justify-center text-xl rounded-full w-11 h-11 bg-red-500/10">🔴</div>
        </div>
      </div>
      <div class="p-5 border rounded-2xl bg-white/5 border-white/5">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="mt-1 text-2xl font-bold text-white">{{ readCount }}</p>
          </div>
          <div class="flex items-center justify-center text-xl rounded-full w-11 h-11 bg-green-500/10">✓</div>
        </div>
      </div>
    </div>
    ...
    <div class="flex flex-wrap items-center gap-2 p-3 mb-5 border rounded-2xl bg-white/5 border-white/5">
      ...
      >Non lues</button>
      <button
        @click="currentFilter = 'salle_occupee'"
        :class="[
          'px-4 py-2 text-sm rounded-xl transition',
      ...
      >🏫 Salles</button>
      <button
        @click="currentFilter = 'prof_indisponible'"
        :class="[
          'px-4 py-2 text-sm rounded-xl transition',
      ...
      >👨‍🏫 Professeurs</button>
      <button
        @click="currentFilter = 'conflit_horaire'"
        :class="[
          'px-4 py-2 text-sm rounded-xl transition',
    ...
    <div v-if="error" class="p-4 mb-4 text-sm text-red-400 border bg-red-500/10 border-red-500/20 rounded-xl">
      {{ error }}
      <button @click="error = null" class="ml-2 text-xs underline">Fermer</button>
    </div>

    <!-- Loading -->
    <div v-if="loading" class="flex flex-col items-center justify-center p-12 border rounded-2xl bg-white/5 border-white/5">
      <div class="w-10 h-10 border-4 border-gray-700 rounded-full border-t-gt-blue animate-spin"></div>
      <p class="mt-4 text-sm text-gray-500">Chargement des notifications...</p>
    </div>

    <!-- Liste vide -->
    <div v-else-if="filteredNotifications.length === 0" class="p-12 text-center border rounded-2xl bg-white/5 border-white/5">
      <div class="mb-4 text-5xl">🔔</div>
      <h2 class="text-lg font-semibold text-white">Aucune notification</h2>
    ...
    <div v-else class="space-y-3">
      ...
      >
        <!-- Indicateur non lu -->
        <div v-if="!notification.is_read" class="absolute left-0 w-1 h-12 rounded-r-full top-5 bg-gt-blue"></div>
          ...
          <div class="flex-1 min-w-0">
            <div class="flex flex-wrap items-center justify-between gap-2">
              <div class="flex items-center gap-2">
                <h3 class="font-semibold text-white">{{ getNotificationTitle(notification.type) }}</h3>
                <span
            ...
            <p class="mt-2 text-sm leading-6 text-gray-400">{{ notification.message }}</p>

            <div v-if="notification.cours_id || notification.salle_id" class="flex flex-wrap gap-2 mt-3">
              <span v-if="notification.cours_id" class="px-2 py-1 text-xs text-gray-400 rounded-lg bg-white/5">
                📚 Cours #{{ notification.cours_id }}
              </span>
              <span v-if="notification.salle_id" class="px-2 py-1 text-xs text-gray-400 rounded-lg bg-white/5">
            ...
            <div class="flex flex-wrap gap-2 mt-4">
              <button
                ...
                :disabled="processingId === getId(notification)"
              ...
              >
                <span v-if="processingId === getId(notification)">...</span>
                <span v-else>✓ Marquer comme lu</span>
              </button>

              <button
                @click="deleteNotification(notification)"
                :disabled="deletingId === getId(notification)"
              ...
              >
                <span v-if="deletingId === getId(notification)">...</span>
    ...
    <div v-if="filteredNotifications.length > 0" class="mt-5 text-center">
      <p class="text-xs text-gray-600">{{ filteredNotifications.length }} notification(s) affichée(s)</p>
    </div>
  </div>
</template>
