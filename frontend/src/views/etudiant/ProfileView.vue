...
<template>
  <div class="max-w-4xl mx-auto space-y-8 pb-12">
    ...
    <template v-else>
      <!-- Success Banner -->
      <div v-if="saveSuccess" class="flex items-center gap-3 px-5 py-3 rounded-xl bg-green-500/10 border border-green-500/30 text-green-400 text-sm font-medium">
        <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>
        Profil mis à jour avec succès !
      </div>

      <!-- Error Banner -->
      <div v-if="etudiantStore.error" class="flex items-center gap-3 px-5 py-3 rounded-xl bg-red-500/10 border border-red-500/30 text-red-400 text-sm font-medium">
        <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
        {{ etudiantStore.error }}
      </div>

      <!-- Profile Header -->
      <div class="bg-gt-card border border-white/5 rounded-[2rem] overflow-hidden shadow-xl">
        <!-- Cover Photo -->
        <div class="h-36 relative bg-gradient-to-r from-blue-700 via-indigo-700 to-purple-800">
        ...
        <div class="px-8 pb-8 relative">
          <div class="flex flex-col sm:flex-row gap-6 items-start sm:items-end -mt-16 mb-6">
            <!-- Avatar -->
            <div class="w-32 h-32 rounded-[2rem] bg-gradient-to-br from-blue-600 to-indigo-700 border-4 border-gt-card shadow-2xl flex items-center justify-center text-4xl font-bold text-white shrink-0 relative overflow-hidden group">
              ...
              <label v-if="isEditing" class="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center cursor-pointer cursor-pointer">
              ...
              <div v-if="!isEditing" class="absolute bottom-1 right-1 w-5 h-5 bg-green-500 rounded-full border-2 border-gt-card"></div>
            </div>
            ...
            <div class="flex-1 pb-2">
              ...
              <p class="text-gray-400 flex items-center gap-2 mb-1">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"></path></svg>
                {{ profile.email }}
              </p>
              <p v-if="profile.filiere" class="text-gray-500 text-sm flex items-center gap-2">
          ...
          <!-- Bio -->
          <div v-if="profile.bio || isEditing" class="mt-2 border-t border-white/5 pt-4">
            <p v-if="!isEditing" class="text-gray-400 text-sm leading-relaxed">{{ profile.bio || 'Aucune biographie.' }}</p>
            <textarea v-else v-model="form.bio" rows="2" placeholder="Votre biographie..."
              class="w-full bg-gray-900/60 border border-white/10 rounded-xl p-3 text-sm text-white placeholder-gray-500 resize-none outline-none focus:border-blue-500/50 transition-colors">
            </textarea>
          </div>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div class="md:col-span-2 space-y-6">
          ...
          <!-- Personal Info -->
          <div class="bg-gt-card border border-white/5 rounded-[2rem] p-6 shadow-lg">
            <h3 class="text-base font-bold text-white mb-5 pb-3 border-b border-white/5 flex items-center gap-2">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div class="space-y-1.5">
                <label class="text-xs font-medium text-gray-500 uppercase tracking-wider">Prénom</label>
          ...
          <div class="bg-gt-card border border-white/5 rounded-[2rem] p-6 shadow-lg">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div class="space-y-1.5">
                <label class="text-xs font-medium text-gray-500 uppercase tracking-wider">Filière</label>
              ...
              <div class="space-y-1.5">
                <label class="text-xs font-medium text-gray-500 uppercase tracking-wider">Niveau</label>
              ...
              <div class="space-y-1.5">
                <label class="text-xs font-medium text-gray-500 uppercase tracking-wider">Date d'inscription</label>
              ...
              <div class="space-y-1.5">
                <label class="text-xs font-medium text-gray-500 uppercase tracking-wider">N° Étudiant</label>
        ...
        <div class="space-y-6">
          <!-- Status Card -->
          <div class="bg-gradient-to-br from-indigo-600 to-purple-800 rounded-2xl p-6 shadow-lg text-center relative overflow-hidden">
            ...
            <div class="relative z-10">
              ...
              <p class="text-indigo-200 text-xs font-semibold uppercase tracking-wider mb-1">Statut du compte</p>
              <p class="text-2xl font-bold text-white">{{ profile.isActive ? 'Actif' : 'Inactif' }}</p>
              <p class="text-indigo-200 text-xs mt-2">Inscrit depuis {{ formatDate(profile.joinDate || profile.createdAt) }}</p>
            </div>
          </div>

          <!-- Quick Stats -->
          <div class="bg-gt-card border border-white/5 rounded-2xl p-5">
            <h3 class="text-sm font-bold text-white mb-4 uppercase tracking-wider">Informations rapides</h3>
            <div class="space-y-3">
          ...
          <!-- Security -->
          <div class="bg-gt-card border border-white/5 rounded-2xl p-5">
            <h3 class="text-sm font-bold text-white mb-4 uppercase tracking-wider text-center">Sécurité</h3>
