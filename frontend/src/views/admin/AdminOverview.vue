  ...
  <div class="pt-8">
    ...
    <div class="grid grid-cols-2 gap-4 mb-10 lg:grid-cols-4">
      <RouterLink v-for="c in cards" :key="c.key" :to="c.to"
        class="relative p-6 border rounded-2xl transition-all group overflow-hidden cursor-pointer"
        :class="`${colorMap[c.color].bg} ${colorMap[c.color].border} hover:scale-[1.02]`">
        <div class="flex items-center justify-between mb-4">
          <!-- Icon -->
          <div class="flex items-center justify-center w-11 h-11 rounded-xl border"
            :class="`${colorMap[c.color].bg} ${colorMap[c.color].border}`">
            <!-- Users -->
            <svg v-if="c.icon === 'users'" class="w-5 h-5" :class="colorMap[c.color].text" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z" />
            </svg>
            <!-- Building -->
            <svg v-if="c.icon === 'building'" class="w-5 h-5" :class="colorMap[c.color].text" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" />
            </svg>
            <!-- Book -->
            <svg v-if="c.icon === 'book'" class="w-5 h-5" :class="colorMap[c.color].text" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" />
            </svg>
            <!-- Calendar -->
            <svg v-if="c.icon === 'calendar'" class="w-5 h-5" :class="colorMap[c.color].text" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div>
          ...
          <p v-else class="text-3xl font-bold text-white mb-1">{{ stats[c.key] }}</p>
    ...
    <div class="mb-8">
      ...
      <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
        ...
        <RouterLink to="/admin/academic-years"
          class="flex items-center gap-4 p-4 border rounded-2xl bg-white/3 border-white/5 hover:border-white/10 hover:bg-white/5 transition-all group">
          ...
          <svg class="w-4 h-4 ml-auto text-gray-600 group-hover:text-gray-400 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
          </svg>
        </RouterLink>
